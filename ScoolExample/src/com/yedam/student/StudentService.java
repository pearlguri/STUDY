...
public class StudentService {
	...
	// 전공별 성적 합계 및 평균 => DB에서 조회된 정보를 활용해서 만드는 방식
	//Java -> 전공을 확인 -> 배열에 넣어주고 -> 전공별로 합계 또는 평균
	//데이터를 다룰때 DB에서 처리가 다 가능하다면,, DB에서 처리하고 Java로
	//넘겨주는게 훨씬 좋다.
	public void getAnalyze() {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			System.out.println("| 학생수 : " + (int) (list.get(i).getSum() / list.get(i).getAvg()) + "명ㄴ"); // 합계 / 평균 = 학생수
		}
	}
}
